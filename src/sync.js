...
/* Cloud sync via Supabase. The app is local-first: everything lives in localStorage and works without an account.
   ...
   logs in, profile, log and activities are mirrored to the user_data table (see supabase/schema.sql). */
...
export const pullRemote = async (userId) => {
  const { data, error } = await supabase.from("user_data").select("profile, log, activities, updated_at").eq("user_id", userId).maybeSingle();
  if (error) throw error;
...
export const pushRemote = async (userId, { profile, log, activities, updatedAt }) => {
  const { error } = await supabase.from("user_data").upsert({ user_id: userId, profile, log, activities, updated_at: new Date(updatedAt || Date.now()).toISOString() }, { onConflict: "user_id" });
  if (error) throw error;
};
